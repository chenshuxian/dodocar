...
module.exports = {
  ...
  trainBook: async (props) => {
    ...
    let i = 0;
    if (props.name) {
      t.where = { name: props.name };
      // 取得最後一比trainbook id
      lastId = await TB.findAll({
        attributes: ['id'],
        order: [['createdAt', 'DESC']],
        limit: 1,
      });
      console.log(`last ${JSON.stringify(lastId)}`);
      i = lastId[0].id;
      i = i.substr(6);
      i = parseInt(i, 10);
    }

    // console.log(`modules/trainBook ${t}`);

    let teacher = await Teacher.findAll(t);
    ...
    for (var id in teacher) {
      ...
      for (var name in exam) {
        for (var k in TRAINTIME) {
          i++;
          console.log(
            `tId:${teacher[id]['id']},eId:${exam[name]['name']},i:${i}`
          );
          TB.create({
            id: `TB${year}${i}`,
            trainTimeId: k,
